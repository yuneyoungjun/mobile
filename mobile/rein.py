...
class TurtleBotController(Node):
    ...
    def control_turtlebot(self, msg):
        ...
        # ✅ 입력 벡터 생성
        self.input_vector[:121] = self.lidar_data * 20
        ...
        if len(msg.poses) != 0:
            self.linear_velocity = max(min(outputs[2][0][1] * self.v_sale, 0.3), -0.3)+0.05
            self.angular_velocity = max(min(outputs[2][0][0] * self.w_sale, 0.3), -0.3)
        else:
            self.linear_velocity = 0.0
            self.angular_velocity = 0.0
